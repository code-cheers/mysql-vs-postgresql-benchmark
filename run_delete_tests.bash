...
QUERIES_TO_EXECUTE="${QUERIES_TO_EXECUTE:-100000}"
QUERIES_RATE="${QUERIES_RATE:-10000}"
mkdir -p "${RESULTS_DIR}"

usage() {
  cat <<'EOF'
Usage:
  ./run_delete_tests.bash [mysql|postgresql|both]

Examples:
  ./run_delete_tests.bash
  ./run_delete_tests.bash mysql
  ./run_delete_tests.bash postgresql

Optional:
  RESULTS_DIR=result QUERIES_TO_EXECUTE=100000 QUERIES_RATE=10000 ./run_delete_tests.bash
EOF
}
...
run_single_test() {
  ...
  local db_name
  local test_option=6
  local test_case_name="delete_orders_by_id"
